...
- Inside it, fill the constants:
	-  `videoId` with the ID of the video previously uploaded
	- `thumbnailSeekTime` with the timestamp of the frame that will become the video's thumbnail
	- `previewStartTime` with the starting timestamp of the preview (a sort of a trailer for the video)
	- `previewDuration` with the duration of the preview, in seconds
